...
class IRFService {
  ...
  String _generateSavedReportingPersonDocId(
    ...
    final dateStr =
        DateFormat('yyyyMMdd').format(referenceDate ?? DateTime.now());
    final sanitizedFirstName =
        (firstName ?? '').replaceAll(RegExp(r'[^A-Za-z0-9]'), '').toUpperCase();
  ...
  Future<DocumentSnapshot<Map<String, dynamic>>?>
      _getSavedReportingPersonDocSnapshot() async {
  ...
  Future<String?> uploadImage(dynamic imageData, String irfId) async {
    try {
      ...
      final Reference storageRef =
          _storage.ref().child('irf-app-images').child('$irfId.jpg');
      ...
      if (e.toString().contains('unauthorized')) {
        throw Exception(
            'User not authorized to upload images. Please check your permissions.');
      }
    ...
    try {
      ...
      for (final doc in incidentsQuery.docs) {
        ...
        allCases.add({
          ...
          'dateCreated': _formatTimestamp(
              incidentDetails['createdAt'] ?? data['createdAt']),
          'status': status,
          'source': 'incidents',
          'pdfUrl': data['pdfUrl'],
          'rawData': data,
  ...
  Future<bool> saveReportingPersonData(
      Map<String, dynamic> reportingPersonData) async {
    try {
      if (currentUserId == null) {
      ...
      final targetFirstName = (reportingPersonData['firstName'] ??
              existingReportingData?['firstName'])
          ?.toString();
      ...
      if (existingSnapshot != null && existingSnapshot.id != resolvedDocId) {
        ...
        print(
            'Migrated saved reporting person data for user: $currentUserId from ${existingSnapshot.id} to $resolvedDocId');
        return true;
      }
      ...
      if (isNewDoc) {
        ...
        print(
            'Successfully saved reporting person data for user: $currentUserId with ID: ${targetDocRef.id}');
      } else {
        await targetDocRef.update(payload);
        print(
            'Successfully updated existing reporting person data for user: $currentUserId with ID: ${targetDocRef.id}');
      }
      return true;
    } catch (e) {
  ...
  Future<bool> clearSavedReportingPersonData() async {
    try {
      ...
      if (existingSnapshot != null) {
        await existingSnapshot.reference.delete();
        print(
            'Successfully cleared saved reporting person data for user: $currentUserId');
        return true;
      }
  ...
  Future<bool> updateSavedReportingPersonData(
      Map<String, dynamic> reportingPersonData) async {
