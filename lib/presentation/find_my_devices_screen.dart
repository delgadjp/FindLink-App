  ...
  Future<void> _loadDevices() async {
    try {
      ...
      if (userQuery.docs.isNotEmpty) {
        ...
        try {
          final locations =
              await _locationService.getLocationHistory(user.uid, limit: 1);
          if (locations.isNotEmpty) {
            lastLocation = locations.first;
            print(
                'Current user location found: ${lastLocation.latitude}, ${lastLocation.longitude}');
          } else {
      ...
      print(
          'Found ${trustedContactsQuery.docs.length} trusted contacts with location access');
      ...
      print(
          'DEBUG: Found ${allTrustedContactsQuery.docs.length} total trusted contacts');
      for (var doc in allTrustedContactsQuery.docs) {
        final data = doc.data();
        print(
            '  Contact: ${data['name']}, canAccessLocation: ${data['canAccessLocation']}, isVerified: ${data['isVerified']}');
      }
      ...
      for (var contactDoc in trustedContactsQuery.docs) {
        ...
        try {
          ...
          if (contactUserQuery.docs.isNotEmpty) {
            ...
            print(
                '  - Contact user found: ${contactUserData['name'] ?? contactUserData['displayName']}');
            ...
            if (contactUserData['findMeEnabled'] == true) {
              ...
              try {
                print(
                    '  - Getting location history for contactUserId: $contactUserId');
                ...
                if (contactUserId == user.uid) {
                  print(
                      '  - ❌ ERROR: contactUserId matches current user! This should not happen.');
                  print('  - Contact name: ${contactData['name']}');
                ...
                final locations = await _locationService
                    .getLocationHistory(contactUserId, limit: 1);
                if (locations.isNotEmpty) {
                  contactLocation = locations.first;
                  print(
                      '  - Contact location found: ${contactLocation.latitude}, ${contactLocation.longitude}');
                  print(
                      '  - Contact location address: ${contactLocation.address}');
                  print(
                      '  - Contact location userId field: ${contactLocation.userId}');

                  // CRITICAL VALIDATION: Ensure the location actually belongs to the contact
                  if (contactLocation.userId != contactUserId) {
                    print(
                        '  - ❌ CRITICAL BUG DETECTED: Location userId (${contactLocation.userId}) != contactUserId ($contactUserId)');
                    print(
                        '  - This means the wrong location is being returned!');
                    // Don't use this location data as it's incorrect
                    contactLocation = null;
                  } else {
                    print(
                        '  - ✅ Location validation passed: userId matches contactUserId');
                  }
                } else {
            ...
            print(
                '  - Contact user not found for contactUserId: $contactUserId');
          }
        } catch (e) {
      ...
      if (trustedContactsQuery.docs.isEmpty) {
        print('');
        print(
            '=== EXPERIMENTAL FIX: Checking for mutual trusted contact relationships ===');
        ...
        try {
          ...
          print(
              'Found ${mutualContactsQuery.docs.length} users who have current user as trusted contact');

          for (var mutualContactDoc in mutualContactsQuery.docs) {
            final mutualContactData = mutualContactDoc.data();
            final mutualOwnerId = mutualContactData[
                'userId']; // The person who added current user as contact

            print('Processing mutual relationship:');
            print('  - Mutual owner userId: $mutualOwnerId');
            print(
                '  - Current user is their contact: ${mutualContactData['contactUserId']}');
            ...
            if (reverseContactQuery.docs.isNotEmpty) {
              ...
              print(
                  '  - ✅ Found mutual relationship! Current user also has them as contact');
              ...
              if (mutualUserQuery.docs.isNotEmpty) {
                ...
                if (mutualUserData['findMeEnabled'] == true) {
                  ...
                  try {
                    final locations = await _locationService
                        .getLocationHistory(mutualOwnerId, limit: 1);
                    if (locations.isNotEmpty) {
                      mutualLocation = locations.first;
                      print(
                          '  - Mutual contact location found: ${mutualLocation.latitude}, ${mutualLocation.longitude}');
                    }
                  } catch (e) {
              ...
              print(
                  '  - No reverse relationship found (one-way relationship only)');
            }
          }
        } catch (e) {
      ...
      for (var device in devices) {
        ...
        if (lastLocation != null) {
          if (lastLocation.userId != deviceId) {
            print(
                '❌ CRITICAL ERROR: Device $deviceName has location data from user ${lastLocation.userId} instead of $deviceId');
                ...
                child: _devices.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final lastLocation =
                              device['lastLocation'] as LocationData?;
