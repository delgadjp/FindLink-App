...
class ImageProcessor {
  ...
  static Future<Uint8List> processImageForDetection(dynamic imageData) async {
    try {
      ...
      print(
          'Image processed successfully. Original size: ${imageBytes.length} bytes, Processed size: ${processedBytes.length} bytes');
  ...
  static img.Image _optimizeResolution(img.Image image) {
    ...
    if (maxDimension > targetMaxDimension) {
      ...
      print(
          'Scaling image from ${width}x${height} to ${newWidth}x${newHeight}');
    ...
    // If image is too small, scale it up slightly for better detection
      ...
      print(
          'Upscaling image from ${width}x${height} to ${newWidth}x${newHeight}');
      return img.copyResize(image,
          width: newWidth,
          height: newHeight,
  ...
  static img.Image _adjustContrastAndBrightness(img.Image image) {
    ...
    double avgBrightness =
        totalBrightness / (pixelCount / 25); // Adjusted for sampling
    ...
    print(
        'Applying brightness: ${brightnessAdjustment.toStringAsFixed(1)}, contrast: ${contrastAdjustment.toStringAsFixed(2)}');
  ...
  static Map<String, dynamic> analyzeImageQuality(img.Image image) {
    ...
    int brightPixels =
        brightnessLevels.sublist(200, 256).reduce((a, b) => a + b);
    int sampleSize =
        (totalPixels / 4).round(); // Due to sampling every 2nd pixel
    ...
    if (width < 640 || height < 480) {
      recommendations.add(
          'Image resolution is low. Consider taking a higher resolution photo.');
      qualityRating = 'Fair';
    }

    if (darkPercentage > 60) {
      recommendations
          .add('Image appears dark. Try taking photo in better lighting.');
      qualityRating = 'Poor';
    }

    if (brightPercentage > 40) {
      recommendations
          .add('Image appears overexposed. Avoid direct bright light.');
    ...
    return {
      ...
      'isOptimalSize':
          width >= 640 && height >= 480 && width <= 2048 && height <= 2048,
    };
  }
}
